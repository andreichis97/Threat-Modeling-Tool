...
def main():
    ...
    for q_idx, question in enumerate(questions_truths_list_owasp):
        question_text = question["question"]
        print(f"\n\n{'#'*70}")
        print(f"QUESTION {q_idx + 1}/{len(questions_truths_list_owasp)}: {question_text[:80]}...")
        ...
        try:
            ...
            results_context.append(record_context)
            print(f"\n  ✓ Done in {total_time:.2f}s | Cost: ${tracker.total_cost:.6f}")
            print(f"  Response: {response[:150]}...")

            # Write after each question for crash recovery
            write_to_json("./results/generated_26_03_ontology_traversal_run3.json", results) #here
            write_to_json("./results/kg_context_generated_26_03_ontology_traversal_run3.json", results_context) #here
            ...
            results_context({"context_graph": ""})
            write_to_json("./results/generated_30_03_ontology_traversal_owasp_run3.json", results) #here
            write_to_json("./results/kg_context_generated_30_03_ontology_traversal_owasp_run3.json", results_context) #here
            continue
